...
def get_chat(chat_id):
    ...
    for row in chats:
        ...
        chat_data['entries'].append({'type': entry_type, 'body': body, 'chat_entry_id':chat_entry_id})
...
def continue_chat(id):
    ...
    past_messages = [{'role': sub_dict['type'], 'content': sub_dict['body']} for sub_dict in  chat_data['entries']]
    if chat_data["model"] == "gpt-3.5":
        ...
        if api_key is None or api_key['openai_key'] == "" or api_key['openai_key'] is None:
            ...
            try:
                api_key = api_key['openai_key']
                output = generate_openai_response(api_key=api_key, user_prompt=body, past_messages=past_messages)
            except Exception as e:
                flash(e, 'error')
                return render_template('chat/chat_view.html', chat_data=chat_data)
            else:
                if not output["valid"]:
                    ...
                    # if correct, register the chat and individual chat entries
                    chat_id = register_chat(chat_data["model"], body, output["output"], provided_chat_id=id)
                    # fetch chat data for this newly inserted chat id
                    return redirect(url_for("chat.load_chat", id=chat_id))
...
def regenerate_chat(id):
    ...
    latest_system_chat_id = [sub_dict["chat_entry_id"] for sub_dict in  chat_data['entries']][-1]
    if chat_data["model"] == "gpt-3.5":
        api_key = get_db().execute('SELECT openai_key'
                                   ' FROM user_settings'
                                   ' WHERE id = ?',
        ...
        if api_key is None or api_key['openai_key'] == "" or api_key['openai_key'] is None:
            ...
            try:
                api_key = api_key['openai_key']
                output = generate_openai_response(api_key=api_key, user_prompt=user_message, past_messages=past_messages)
            except Exception as e:
                flash(e, 'error')
                return render_template('chat/chat_view.html', chat_data=chat_data)
            else:
